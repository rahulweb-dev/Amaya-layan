...
const SLIDES = [
  { label: 'Community', src: '/images/social-lounge.jpg' },
  { label: 'Wellness', src: '/images/gym.jpg' },
  { label: 'Nature', src: '/images/garden-walk.jpg' },
  { label: 'Spaces', src: '/images/courtyard-pool.jpg' },
];
...
export default function WorldClassProject() {
  ...
  return (
    <section
      ref={sectionRef}
      className='bg-[#23384A] py-20 md:py-28 xl:py-16 overflow-hidden sm:block hidden'
    >
      <div className='max-w-325 mx-auto px-5 sm:px-8 md:px-16 xl:px-24'>
        <p
          ref={labelRef}
          className='text-center text-limestone text-[19px] uppercase tracking-[0.35em] mb-10 md:mb-14 md:text-[18px]'
        >
          Why Amaya
        </p>
        ...
        <h2
          ref={headlineRef}
          className='text-center uppercase text-limestone font-light tracking-[0.03em] leading-[1.2] text-[35px] md:text-[48px] max-w-275 mx-auto mb-20 md:mb-28 xl:mb-16'
        >
          Designed around a<br/> different idea of home
        </h2>

        <div className='grid grid-cols-1  gap-14 md:gap-20 xl:gap-28 items-center'>
          ...
          {/* <div ref={leftColRef} className="flex justify-center md:justify-start">
            ...
            >
              ...
              <div className="absolute bottom-0 left-0 right-0 flex items-end justify-center pb-10 z-20">
                <span ref={slideNameRef} className="text-limestone uppercase tracking-[0.35em] text-sm md:text-base font-light">
                  {activeLabel}
                </span>
              </div>
          ...
          >
            <p className='text-limestone/70 font-light leading-[1.85] text-[16px] md:text-[18px]'>
              Thoughtful homes for easier everyday living. A place where the
              rhythm of each day is yours to set, and where community, care, and
              nature are simply close at hand
            </p>

            <div className='text-center'>
              <p className='text-limestone/85 font-light text-[16px] md:text-[18px] leading-[1.7] mb-5'>
                Not a facility. Not a resort. A residential community built for
                independence, with everything that makes independence feel
                effortless:
              </p>
              ...
              <ul className='flex flex-col items-center gap-2.5'>
                {BULLETS.map((bullet) => (
                  <li
                    key={bullet}
                    className='flex items-start gap-3 text-center text-limestone/60 text-[16px] md:text-[18px] font-light leading-[1.65] max-w-5xl'
                  >
                    <span className='mt-3.5 w-1.5 h-1.5 rounded-full bg-brass/60 shrink-0' />
                    <span>{bullet}</span>
                  </li>
                ))}
              </ul>
            </div>
            <p className='text-limestone/40 font-light text-[16px] md:text-[18px] italic leading-[1.7]  border-brass/35 pl-5 mt-2'>
              &ldquo;A life that is social when you want it, and quiet when you
              need it&rdquo;
            </p>
          </div>
        </div>
