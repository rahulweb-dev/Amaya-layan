...
export default function Amenities() {
  ...
  function onEnter(idx: number) {
    ...
    if (titleRefs.current[idx])
      gsap.to(titleRefs.current[idx], { scale: 1.035, color: '#E7D8C6', duration: 0.4, ease: 'power2.out', overwrite: true });
  }
  ...
  function onLeave(idx: number) {
    ...
    if (titleRefs.current[idx])
      gsap.to(titleRefs.current[idx], { scale: 1, color: 'rgba(231,216,198,0.35)', duration: 0.4, ease: 'power2.out', overwrite: true });
  }
  ...
  function openPanel(idx: number) {
    ...
    titleRefs.current.forEach(el => { if (el) gsap.to(el, { scale: 1, color: 'rgba(231,216,198,0.35)', duration: 0.2 }); });
  ...
  function navigate(dir: 1 | -1) {
    ...
    setActiveIdx(prev => {
      ...
      gsap.timeline({ onComplete: () => { isAnimating.current = false; } })
        .to([panelBodyRef.current, peekRef.current], { opacity: 0, y: dir > 0 ? -22 : 22, duration: 0.28, ease: 'power2.in' })
  ...
  return (
    <section ref={sectionRef} className="relative bg-[#23384A] overflow-hidden min-h-screen">
            ...
            >
              ...
              >
                <h3
                  ...
                  style={{ fontSize: 'clamp(2.4rem, 6.5vw, 6.5rem)', color: 'rgba(231,216,198,0.35)' }}
                >
                  {item.title}
                </h3>
              </button>
