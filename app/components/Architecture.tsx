...
export default function Architecture() {
  ...
  useEffect(() => {
    const ctx1 = gsap.context(() => {
      ...
      gsap.from(headingRef.current, { y: 60, opacity: 0, duration: 1.2, ease: 'power3.out', scrollTrigger: st1 });
      gsap.from(smallImgRef.current, { x: -50, opacity: 0, duration: 1, ease: 'power3.out', scrollTrigger: { ...st1, start: 'top 68%' } });
      gsap.from(textRef.current, { y: 30, opacity: 0, duration: 0.9, ease: 'power3.out', delay: 0.15, scrollTrigger: { ...st1, start: 'top 65%' } });
      gsap.from(bigImgRef.current, { x: 80, opacity: 0, duration: 1.1, ease: 'power3.out', scrollTrigger: { ...st1, start: 'top 68%' } });
    }, sec1Ref);

    const ctx2 = gsap.context(() => {
      const st2 = { trigger: sec2Ref.current, start: 'top 72%' };
      gsap.from(poolImgRef.current, { x: -70, opacity: 0, duration: 1.2, ease: 'power3.out', scrollTrigger: st2 });
      gsap.from(ecoTextRef.current, { x: 50, opacity: 0, duration: 1, ease: 'power3.out', delay: 0.2, scrollTrigger: st2 });
    }, sec2Ref);
  ...
  return (
    <>
      {/* ── SECTION 1: Homes ── */}
      <section ref={sec1Ref} className="bg-surface-alt pt-20 md:pt-28 xl:pt-36 overflow-hidden">

        <div className="max-w-425 mx-auto px-5 sm:px-8 md:px-16 xl:px-24 mb-12 md:mb-16 xl:mb-20">
          <h2
            ref={headingRef}
            className="text-navy font-light leading-[0.95] tracking-[0.03em] text-center md:text-left"
            style={{ fontSize: 'clamp(2.2rem, 8vw, 8.5rem)' }}
          >
            Homes That Feel
            <br />Exactly Like Home.
          </h2>
        </div>
        ...
        <div className="flex flex-col md:flex-row items-stretch">
          ...
          <div className="w-full md:w-[42%] shrink-0 px-8 md:px-16 xl:px-24 pb-12 md:pb-0 flex flex-col gap-8 justify-start">
            <div ref={smallImgRef} className="relative overflow-hidden rounded-sm" style={{ aspectRatio: '4/3' }}>
              <Image
            ...
            <div ref={textRef} className="flex flex-col gap-4 pb-16 md:pb-20">
              <h3 className="text-navy font-light uppercase tracking-[0.08em] text-[clamp(1.4rem,2.2vw,2.2rem)]">
                Our Residences
              </h3>
              <p className="text-charcoal/60 font-light text-[13.5px] md:text-[14.5px] leading-[1.85] max-w-105">
              ...
              <div className="flex flex-col gap-2 mt-3">
                {[
                  ...
                  <div key={type} className="flex items-center gap-3 text-charcoal/50 text-[12px] md:text-[13px] font-light">
      ...
      <section ref={sec2Ref} className="bg-surface overflow-hidden">
        <div className="flex flex-col md:flex-row items-center">
          ...
          >
            ...
            <h2
              className="text-navy font-light uppercase tracking-wider leading-tight"
              style={{ fontSize: 'clamp(1.8rem,3.5vw,3.6rem)' }}
            >
              A Clubhouse That<br />Earns Its Name.
            </h2>
            <p className="text-charcoal/60 font-light text-[13.5px] md:text-[14.5px] leading-[1.85] max-w-110">
            ...
            <ul className="flex flex-col gap-2 mt-1">
              {[
                ...
                <li key={item} className="flex items-start gap-3 text-charcoal/55 text-[13px] md:text-[14px] font-light">
                  <span className="mt-2 w-1 h-1 rounded-full bg-brass/40 shrink-0" />
                  {item}
                </li>
              ))}
            </ul>
