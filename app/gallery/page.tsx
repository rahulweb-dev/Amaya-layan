...
type Category = 'All' | 'Architecture' | 'Landscape' | 'Amenities' | 'Community' | 'Wellness' | 'Location';

const FILTERS: Category[] = ['All', 'Architecture', 'Landscape', 'Amenities', 'Community', 'Wellness', 'Location'];
...
function LightboxModal({
  ...
  return (
    ...
    >
      <button
        className="absolute top-5 right-6 text-white/50 hover:text-white text-[28px] font-light transition-colors duration-200 z-10"
        onClick={onClose}
        aria-label="Close"
      >
        ×
      ...
      <button
        className="absolute left-4 md:left-8 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full border border-white/20 flex items-center justify-center text-white/50 hover:text-white hover:border-white/50 transition-all duration-200 z-10"
      ...
      <button
        className="absolute right-4 md:right-8 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full border border-white/20 flex items-center justify-center text-white/50 hover:text-white hover:border-white/50 transition-all duration-200 z-10"
      ...
      >
        <div className="relative w-full overflow-hidden" style={{ maxHeight: '70vh', aspectRatio: '16/10' }}>
          <Image
            src={item.src}
            alt={item.alt}
            fill
        ...
        <div className="flex items-center justify-between px-1">
          <div>
            <p className="text-white/80 text-[14px] font-light">{item.caption}</p>
            <p className="text-white/35 text-[11px] uppercase tracking-[0.3em] mt-0.5">{item.category}</p>
          </div>
          <p className="text-white/25 text-[11px] uppercase tracking-[0.2em]">Press ← → to navigate</p>
        </div>
      </div>
    </div>
  );
...
export default function GalleryPage() {
  ...
  const prevLightbox = () => setLightboxIdx((i) => (i !== null ? (i - 1 + filtered.length) % filtered.length : 0));
